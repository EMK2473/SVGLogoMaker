// object.assign research and organize
// ...arguments(rest)
class Shape {
...
class Triangle extends Shape {
  constructor(color, text, textColor, textSize, svgW, svgH, triPoints, textX, textY) {
  ...
  generateShape() {
    return `
      <svg width="${this.svgW}" height="${this.svgH}" version="1.1" xmlns="http://www.w3.org/2000/svg">
        <polygon points="${this.triPoints}" fill="#${this.color}" stroke="#${this.color}" stroke-width="5"/>
        <text x="${this.textX}" y="${this.textY}" font-size="${this.textSize}" text-anchor="middle" fill="#${this.textColor}">${this.text}</text>
      </svg>
    `;
  }
