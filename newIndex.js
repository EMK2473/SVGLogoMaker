...
const questions = [
  ...
  },
  {
    ...
    type: "input",
    name: "squareXPlot",
    message: "What is x plot value for your square?",
    when: (answers) => answers.shape === "square",
    validate(input) {
        if (/^[0-9]{1,2}$/.test(input)) {
          return true;
        }
        throw Error('Please provide a valid x plot for your square. \r\n It can only be a maximum of 2 characters \r\n May only contain (0-9)');
      },
  },{
    type: "input",
    name: "squareYPlot",
    message: "What is the y plot value of your square?",
    when: (answers) => answers.shape === "square",
    validate(input) {
        if (/^[0-9]{1,2}$/.test(input)) {
          return true;
    ...
    type: "input",
    name: "squareX1Plot",
    message: "What is x1 plot value for your square?",
    when: (answers) => answers.shape === "square",
    validate(input) {
        if (/^[0-9]{1,2}$/.test(input)) {
          return true;
        }
        throw Error('Please provide a valid x1 plot for your square. \r\n It can only be a maximum of 2 characters \r\n May only contain (0-9)');
      },
  },{
    type: "input",
    name: "triA",
    message: "What is the area value of your triangle?",
    ...
    type: "input",
    name: "triSL",
    message: "What is the side_length value of your triangle?",
    when: (answers) => answers.shape === "triangle",
    validate(input) {
        if (/^[0-9]{1,2}$/.test(input)) {
          return true;
    ...
    type: "input",
    name: "text",
    message: "What is the text value of your shape? Please enter up to 3 characters to print on your logo. \r\n (A-F, a-f) accepted \r\n'",
    validate(input) {
  ...
  type: 'input',
  name: 'textSize',
  message: 'Please enter a text size value to color your text. \r\n (0-9) accepted \r\n',
  validate(input) {
    if (/^[0-9]{1,2}$/.test(input)) {
      return true;
    }
    throw Error('Please provide a text size value. \r\n It can only be 2 numbers \r\n May only contain (0-9)');
  },
}
];
